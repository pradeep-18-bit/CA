...
export default function PendingInvoicesPageStaff() {
  ...
  // LOAD ONLY PENDING + OVERDUE INVOICES (wrapped in useCallback)
    ...
    try {
      ...
      const filtered = data
        ...
        .map((inv) => ({
          ...
          amount: "₹" + calculateTotalAmount(inv.invoiceItems).toLocaleString("en-IN"),
          dueDate: inv.dueDate,
          status: inv.status,
        }));
  ...
  }, [API_URL, calculateTotalAmount]); // dependencies okay (static + memoized)

  useEffect(() => {
    loadInvoices();
  }, [loadInvoices]); // ✔ No warnings now
            ...
            {currentInvoices.map((invoice, i) => (
              ...
              >
                ...
                <td
                  style={{
                    padding: "10px 16px",
                    color: invoice.status === "Overdue" ? "red" : "#f39c12",
                    fontWeight: "600",
                  }}
                >
                  {invoice.status}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  backgroundColor: num === currentPage ? "#073D7F" : "#fff",
